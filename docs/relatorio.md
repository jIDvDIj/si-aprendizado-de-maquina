...
Para cada um dos 28 modelos foram geradas a matriz de confusão e as métricas Acurácia, Precisão, Recall e F1-Score (as três últimas referidas à classe positiva, AVC). A leitura clínica de cada uma:
...
Com 24 combinações de KNN testadas (contra 6 na primeira rodada), a tabela abaixo mostra os destaques; a lista completa está em `resultados/resultados_comparativos.csv`.
...
| ⋮ (demais 16 combinações de KNN, Recall entre 0,20 e 0,40) | | | | |
...
*(Heatmaps das 28 matrizes de confusão em `resultados/fig_matrizes_knn_uniforme.png`, `resultados/fig_matrizes_knn_distancia.png` e `resultados/fig_matrizes_arvore.png`.)*
...
O projeto cumpriu as três fases propostas: pré-processamento fundamentado, exploração manual de 28 configurações de dois algoritmos (incluindo uma segunda rodada de ajuste fino do KNN, que elevou seu Recall de 40% para 52%) e avaliação rigorosa centrada no contexto clínico. A **Árvore de Decisão com `max_depth=5`**, treinada sobre dados balanceados por SMOTE, foi eleita o modelo recomendado por detectar **82% dos casos de AVC** do teste — 41 de 50 pacientes, contra 52% do melhor KNN e 0% do baseline que "vence" em acurácia; ambos os modelos ficam disponíveis na interface. Os resultados ilustram as duas lições metodológicas centrais do trabalho: **em dados desbalanceados, a acurácia não mede utilidade** — e a métrica de avaliação deve ser escolhida pelo custo real dos erros do domínio, que aqui é o falso negativo.
